...
function animateBars() {
  bars.forEach((bar) => {
    ...
    if (rect.top < window.innerHeight - 100) {
      if (bar.querySelector(".fill") && !bar.querySelector(".fill").style.width) {
        setTimeout(() => {
...
// We attach scroll to the individual slides since the body doesn't scroll
...
// Also check on window load for the active slide
window.addEventListener('load', () => {
    animateBars();
    animateOnScroll();
});
...
if (contactForm) {
  contactForm.addEventListener("submit", (e) => {
    e.preventDefault();
    // Use a custom modal here if you build one, instead of alert()
